...
// Aruba has a single product (the ED Card). Kept as an array so the
// pricing UI loops the same way as India's — easier to reconcile when
// we unify both countries' settings pages.
const VISA_CODES = ['ED_CARD'];

// Aruba doesn't have visa-type sub-purposes the way India's Business
// visa does; the Aruba spec exposes "Purpose of Visit" as a regular
// dropdown field on the finish form (Vacation, Business, Wedding, etc.)
// rather than as a sub-form gate. Empty list keeps the existing UI
// scaffolding from breaking.
const KNOWN_PURPOSES: string[] = [];
const VISA_LABELS: Record<string, string> = {
  ED_CARD: 'Aruba ED Card',
};
...
export default function SettingsPage() {
  ...
  const [appSchema, setAppSchema] = useState<ApplicationSchema>(defaultSchema('ARUBA'));
  const [appSchemaSaved, setAppSchemaSaved] = useState<ApplicationSchema>(defaultSchema('ARUBA'));
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      try {
        const cRes = await fetch('/api/settings/custom-emails?country=ARUBA');
        if (cRes.ok) {
      ...
      try {
        const sRes = await fetch('/api/settings/custom-statuses?country=ARUBA');
        if (sRes.ok) {
      ...
      try {
        const aRes = await fetch('/api/settings/application-schema?country=ARUBA');
        if (aRes.ok) {
          const aData = await aRes.json();
          const loaded: ApplicationSchema = {
            country: aData.country || 'ARUBA',
        ...
        <div className="admin-main" style={{ maxWidth: '100%' }}>
          <div style={{ padding: '4rem 1.5rem', textAlign: 'center', maxWidth: '600px', margin: '0 auto' }}>
            ...
            <p style={{ color: '#6b7280', fontSize: '0.9rem' }}>
              Aruba settings (pricing, emails, status labels, application schema, bot config) are restricted to owner accounts.
            </p>
      ...
      <div className="admin-main" style={{ maxWidth: '100%' }}>
        <div style={{ padding: '1.5rem', maxWidth: '1200px', margin: '0 auto', width: '100%' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <div>
              <h1 style={{ fontSize: '1.6rem', fontWeight: 700, marginBottom: '0.25rem' }}>🇦🇼 Aruba Settings</h1>
              <p style={{ color: '#6b7280', fontSize: '0.9rem' }}>Control prices, email templates, status labels, and general site settings for Aruba visas.</p>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              <SettingsCard title="Visa Prices" description="Base price for each visa type (processing surcharge is added separately).">
                <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: '0.6rem 1rem', alignItems: 'center' }}>
                  {VISA_CODES.map(code => {
                    const key = `pricing.AW.visa.${code}`;
              ...
              <SettingsCard title="Processing Speed Surcharges" description="Amount added on top of the base visa price.">
                <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: '0.6rem 1rem', alignItems: 'center' }}>
                  {PROCESSING_CODES.map(code => {
                    const key = `pricing.AW.processing.${code}`;
                    const val = getValue(key);
                    const isDraft = key in drafts;
                    return (
                      <>
              ...
              >
                <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: '0.6rem 1rem', alignItems: 'center' }}>
                  {/* Government fee */}
                  {(() => {
                    const key = 'pricing.AW.fees.government';
                    const val = getValue(key);
                    const isDraft = key in drafts;
                    return (
                      <>
                  ...
                  {/* Transaction fee */}
                  {(() => {
                    const key = 'pricing.AW.fees.transactionPercent';
                    const val = getValue(key);
                    const isDraft = key in drafts;
                    return (
                      <>
              ...
              <SettingsCard title="Price Breakdown Preview" description="How prices combine for 1 traveler at checkout (travelers multiply linearly; transaction fee scales with subtotal).">
                <table style={{ width: '100%', fontSize: '0.9rem' }}>
                  ...
                  <tbody>
                    {VISA_CODES.map(code => {
                      const base = Number(getValue(`pricing.AW.visa.${code}`)) || 0;
                      const std = Number(getValue('pricing.AW.processing.standard')) || 0;
                      const rush = Number(getValue('pricing.AW.processing.rush')) || 0;
                      const sup = Number(getValue('pricing.AW.processing.super')) || 0;
                      const gov = Number(getValue('pricing.AW.fees.government')) || 0;
                      const txPct = Number(getValue('pricing.AW.fees.transactionPercent')) || 0;
                ...
                <CustomEmailEditor
                  ...
                  onSave={async (form) => {
                    const res = await fetch('/api/settings/custom-emails', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ ...form, country: 'ARUBA' }),
                    });
                    if (!res.ok) {
              ...
              onSave={async () => {
                ...
                try {
                  const res = await fetch('/api/settings/application-schema', {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ country: 'ARUBA', sections: appSchema.sections }),
                  });
                  if (res.ok) {
                    const d = await res.json();
          ...
          {tab === 'general' && data && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              ...
              {/* Feature flags — site-wide toggles. Lives here in the Aruba General
                  tab as a convenience so admins have one stop for everything; the
                  flags themselves apply globally, not just to Aruba. */}
              <FeaturesCard />
            </div>
          )}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
        <div>
          <h2 style={{ fontSize: '1rem', fontWeight: 700, margin: 0 }}>📋 Application Schema — Aruba</h2>
    ...
    (async () => {
      try {
        const [botRes, schemaRes] = await Promise.all([
          fetch('/api/settings/bot-mapping?country=ARUBA'),
          fetch('/api/settings/application-schema?country=ARUBA'),
        ]);
        if (botRes.ok) {
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch('/api/settings/bot-mapping', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ country: 'ARUBA', overrides }),
      });
      if (res.ok) {
        setFlash('✓ Saved');
        setDirty(false);
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
        <div>
          <h2 style={{ fontSize: '1rem', fontWeight: 700, margin: 0, display: 'inline-flex', alignItems: 'center', gap: '0.4rem' }}>
            ...
            <span>Bot Mapping — Aruba</span>
          </h2>
          <p style={{ fontSize: '0.82rem', color: '#6b7280', marginTop: '0.2rem', maxWidth: '720px' }}>
            Every gov-site field the Playwright bot fills, across all {catalog.length} steps of the
            Aruban government&apos;s visa flow. For each field, pick which <strong>schema field</strong> the
